HR Analytics Dashboard – Power BI
📊 Project Overview

This Power BI dashboard analyzes employee attrition, satisfaction, and compensation trends to support HR decision-making.

Key objectives:
Identify attrition drivers
Measure employee satisfaction
Analyze compensation fairness
Support data-driven HR decisions

🏠 Home Page – Dashboard Introduction
The Home page provides a high-level overview of the dashboard and key workforce metrics.

Key insights displayed:
Total number of employees
Number of departments and job roles
Quick navigation buttons to detailed analysis pages
Purpose:
To give stakeholders an instant understanding of workforce size and dashboard scope.

📈 Executive Overview-
This page focuses on leadership-level KPIs and trends.

Insights include:
Overall attrition rate
Attrition by department and job role
Headcount trends over time
Business value: Helps HR leaders identify high-risk departments and prioritize retention strategies.

😊 Satisfaction Analysis-
This page analyzes employee engagement and satisfaction levels.

Insights include:
Satisfaction scores by department
Work-life balance trends
Relationship between satisfaction and attrition
Business value: Supports initiatives to improve employee experience and reduce turnover.

💼 Career & Compensation Analysis-
This page evaluates compensation structures and career progression.

Insights include:
Salary distribution by job role
Compensation vs experience
Career growth patterns
Business value: Helps ensure pay equity and improve talent retention.

🛠 Tools & Skills Used-
Power BI
DAX
Data Modeling
Data Visualization
HR Analytics

📂 Screenshots-
All dashboard screenshots are available in the /screenshots folder for visual reference.

👤 Author-
Moulika Viyyapu







Helps HR leaders identify high-risk departments and prioritize retention strategies.

